...
window.addEventListener('DOMContentLoaded', function () {
  ...
  if (document.querySelector('#documentation-menu')) {
...
let updateDocsMenu = () => {
  ...
  document.querySelectorAll('#documentation-menu a:not([href="#' + target.getAttribute('id') + '"])').forEach(function (element) {
    // Unset previous active states on the menu anchors
    element.classList.remove(...element.dataset.active.trim().split(' '));
    // Set inactive states on the menu anchors
    element.classList.add(...element.dataset.inactive.trim().split(' '));
  });
  ...
  if (menuElement) {
    ...
    menuElement.classList.remove(...menuElement.dataset.inactive.trim().split(' '));
  }
}